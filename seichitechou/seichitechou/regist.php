<?php
    ...
    // プリペアド ステートメント：準備段階
    $sql = "INSERT INTO seichi (user_name, path_name, created_at, content, latitude, longitude, likes, delete_code) VALUES (?,?,?,?,?,?,?,?)";
    $stm = $pdo -> prepare($sql);
    ...
    $stm -> bindParam('7', $likes, PDO::PARAM_STR);
    $stm -> bindParam('8', $delete_code, PDO::PARAM_STR);
    ...
    if (is_uploaded_file($_FILES["upfile2"]["tmp_name"])) {
        if (move_uploaded_file($_FILES["upfile2"]["tmp_name"], "files/" . $last_id . ".mp3")) {
            chmod("files/" . $last_id . ".mp3", 0644);
        }
    }
